...
```sh
curl -L -O "https://github.com/kuzmak161-creator/doom-ascii/raw/master/install.sh"
curl -L -O "https://github.com/kuzmak161-creator/doom-ascii/raw/master/doom_ascii.7z"
```
...
| `-kpsmooth <n>` | Set the number of ms a key has to be left depressed for it to count as such. Used to counteract jittery inputs when key repeat delay exceeds frametime. |
...
Most terminals aren't designed for massive throughput, so the game cannot be played at full 320×200 resolution at high frames per second.
...
If the displayed image looks incorrect, you are likely using a terminal that does not support 24-bit RGB. See [this](https://github.com/termstandard/colors) for more details, troubleshooting information, and a list of supported terminals.
